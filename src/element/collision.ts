...
export const determineFocusPoint = (
    ...
    switch (element.type) {
        ...
        case "image":
        case "text":
            point = findFocusPointForRectangulars(
                element,
                focus,
                adjecentPointRel,
            );
            ...
            intersections = corners
                ...
                .concat(
                    corners.flatMap((point) =>
                        getCircleIntersections(point, gap, line),
                    ),
                );
            break;
        case "ellipse":
...
export const getCircleIntersections = (
    ...
    return [
        GA.point(
            (xn + n * discrRoot) / squares,
            (yn - m * discrRoot) / squares,
        ),
        GA.point(
            (xn - n * discrRoot) / squares,
            (yn + m * discrRoot) / squares,
...
export const findFocusPointForEllipse = (
    ...
    const m =
        (-px * b ** 2 +
            orientation *
                py *
                Math.sqrt(Math.max(0, squares - a ** 2 * b ** 2))) /
        squares;
